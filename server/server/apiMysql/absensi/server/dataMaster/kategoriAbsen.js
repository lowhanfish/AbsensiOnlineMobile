...
router.post('/Add', (req, res) => {
    // console.log(req.body)
    var query = `
        INSERT INTO jeniskategori (uraian, keterangan, createdBy, createdAt)
        VALUES
        ('`+req.body.uraian+`', '`+req.body.keterangan+`', '`+req.user._id+`', NOW())
    `

    var akses_menu = req.menu_akses
    const levelAkses = akses_menu.find(({ route }) => route === '/kategoriAbsenDarurat');
...
router.post('/view', (req, res) => {
    ...
    let jml_data = `
        SELECT 
        jeniskategori.* FROM jeniskategori
        WHERE 
    ...
    let view = `
        ...
        WHERE 
        jeniskategori.uraian LIKE '%`+cari+`%' 
        ORDER BY jeniskategori.id DESC
