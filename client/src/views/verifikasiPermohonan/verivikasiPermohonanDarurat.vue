      ...
      <q-card-section class="bg-primary text-white">
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="text-h6  h_titleHead">Verifikasi Permohonan Absen Darurat</div>
          ...
          <!-- =================================================== KONTENT =========================================================== -->
            <table width="100%">
              ...
              <tr :class="'h_table_body '+indicatorColor(data.status).bg" v-for="(data, index) in list_data" :key="data.id">
                ...
                <td>{{data.NIP}}</td>
                <td>{{data.jeniskategori_uraian}}</td>
                    ...
                    <form @submit.prevent="addData">
                        <div class="text-center">
                          ...
                          <div class = "row">
                            ...
                            <div class="col-6 col-md-4" v-for="data in list_lampiran" :key="data.id">

                              <div v-if="loadFilex ==true">
                                <q-img
                                    @click="selectImage(data), mdl_img = true"
                              ...
                              <div v-if="loadFilex ==false">
                                <div v-if="typeFileCheck(data.file) == 'pdf'">
                                  <q-img
                                      @click="selectImage(data), mdl_img = true"
                                      src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZO4jCsocpJ9jBjHbI-jPazImlWU_avBbdxQ&usqp=CAU"
                                      spinner-color="primary"
                                      spinner-size="82px"
                                    />
                                </div>
                                ...
                                <a class="clear_download" :href="$store.state.url.URL_APP+'uploads/'+data.file" target="_blank">Download</a>

                              </div>


                                <!-- <span>{{typeFileCheck(data.file)}}</span> -->
                ...
                <q-card class="mdl-md">
                  <q-card-section class="bg-primary">
                    ...
                    <q-card-section class="q-pt-none">
                      ...
                      <div v-if="typeFileCheck(img.file) == 'pdf' || typeFileCheck(img.file) == 'PDF'" style="width:100%; height:1000px">
                        <div style="width:100%; height:1000px" v-if="loadPDF">
                                <vue-pdf-app :pdf="$store.state.url.URL_APP+'uploads/'+img.file"></vue-pdf-app>
                         </div>
                      </div>

                      <div  v-if="typeFileCheck(img.file) !== 'pdf'">
                        <q-img :src="$store.state.url.URL_APP+'uploads/'+img.file">
...
export default {
  ...
  methods: {
    ...
    selectData : function(data){
      ...
      this.getViewLampiran();

    },



    getViewLampiran : function(){
      this.loadFilex = true;
      fetch(this.$store.state.url.URL_client_lampiranImg + "viewOne", {
          method: "POST",
          headers: {
            "content-type": "application/json",
            authorization: "kikensbatara " + localStorage.token
